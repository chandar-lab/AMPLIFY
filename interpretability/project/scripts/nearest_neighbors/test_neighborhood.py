...
def run_neighborhood_probe(cfg: DictConfig):
    ...
    results = []
    results_file = knn_dir / f"{dataset_name}_{model_shorthand}_{model_layer_num}_{flavour_of_nn}_{distance_metric}_neighbors_multilabel_results.parquet.gz"
    ...
    for treatment, matrix_dict in embedding_treatments.items():
        ...
        # Make a stratified splitter - try to maintain balance of labels
        ...
        split_num = 0 # keep track of split number
        for train_inds, _ in msss.split(dmatrix, labels):

            # Get the stuff at just the split indices
            split_distances = dmatrix[train_inds, :][:,train_inds]
            split_labels = labels[train_inds]

            # Make KNN - naturally multilabel
            nn_model = get_neighbor_model(dataset_name, type= flavour_of_nn, weights=neighbor_weight)

            nn_model.fit(split_distances, split_labels)

            for k in neighbor_ks:
                # Set the number of neighbors
                nn_model.n_neighbors = k
                # Make a prediction
                predicted_labels = nn_model.predict(None)

                distances, indices = nn_model.kneighbors(split_distances, n_neighbors=k, return_distance=True)
                neighbor_labels = split_labels[indices] # Labels of the k-neighbors
                
                if neighbor_weight == 'distance':
                    # Score = Sum(Inverse Distance * Neighbor Label) / Sum(Inverse Distance)
                    weights = 1.0 / (distances + 1e-10) 
                    weights_sum = np.sum(weights, axis=1, keepdims=True)
                    weights_expanded = np.repeat(weights[:, :, np.newaxis], neighbor_labels.shape[2], axis=2)
                    weighted_label_sum = np.sum(weights_expanded * neighbor_labels, axis=1)
                    probs_positive_class = weighted_label_sum / weights_sum 
                else: # 'uniform'
                    probs_positive_class = np.mean(neighbor_labels, axis=1)
                ...
                for metric_name, metric_func in knn_metrics.items():
                    ...
                    if metric_name in ['coverage_error', 'label_ranking_average_precision_score', 'label_ranking_loss']:
                        result_info[metric_name] = metric_func(split_labels, probs_positive_class)
                    else:
                        result_info[metric_name] = metric_func(split_labels, predicted_labels)
